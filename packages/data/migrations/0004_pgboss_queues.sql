...
SELECT pgboss.create_queue('financial.recalculate.dead', '{"policy":"standard","retryLimit":0,"partition":false}'::jsonb);
SELECT pgboss.create_queue('sinking.allocate.dead', '{"policy":"standard","retryLimit":0,"partition":false}'::jsonb);
SELECT pgboss.create_queue('financial.recalculate', '{"policy":"standard","retryLimit":3,"retryDelay":5,"retryBackoff":true,"deadLetter":"financial.recalculate.dead","partition":false}'::jsonb);
SELECT pgboss.create_queue('sinking.allocate', '{"policy":"standard","retryLimit":3,"retryDelay":5,"retryBackoff":true,"deadLetter":"sinking.allocate.dead","partition":false}'::jsonb);
