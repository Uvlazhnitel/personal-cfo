...
The Stage 7 portfolio boundary is specified in [PORTFOLIO_CONTRACT.md](PORTFOLIO_CONTRACT.md). Sharesight User API V2/V2.1 is the selected single-owner provider, with the verified field and capability mapping in [PORTFOLIO_PROVIDER_SHARESIGHT.md](PORTFOLIO_PROVIDER_SHARESIGHT.md). Stage 7.1 adds a native-fetch OAuth client and an explicit manual CLI. Each response is encrypted and committed before validation; normalization then records stable source revisions and advances only application-owned scan continuation. Full-history windows detect historical revisions because Sharesight has no incremental cursor. Cash inclusion has three states and normalization emits exactly one Net Worth projection; holdings and provider P/L remain reconciliation evidence. The financial engine imports neither provider nor integration code, and Stage 7.1 does not activate evidence in canonical financial tables.
