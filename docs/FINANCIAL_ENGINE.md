...
Historical checkpoints contain only effective-dated planning and coverage context. The orchestrator derives flows, ambiguities, cash reconciliations, Sinking Funds, allocations, and spending observations from the root canonical history at the inclusive checkpoint cutoff. Included flows must match booked transactions; observations are included only with their canonical flow. Every derived collection is sorted by effective instant and stable identity, so optimistic checkpoint metadata cannot omit canonical evidence or make input order authoritative.
...
A cash reconciliation remains an active investability ambiguity through every cutoff before its resolution instant. A valid resolution becomes effective only when `resolvedAt <= cutoff`; the earlier reversal-transaction time alone never clears it. Before any active projection is built, the engine validates the original audited adjustment and either proves same-transaction reclassification or proves a distinct booked reversal whose entries on the reconciliation account exactly negate the variance in the same currency. Resolved adjustment facts remain in canonical audit history but leave the active metric projection together, preserving CCR validation without treating them as income or consumption.
