...
Migration `0010` adds the Enable Banking consent and manual evidence boundary. Connections store only encrypted current-session metadata and the actual provider expiry. Authorization attempts store single-use state hashes for 15 minutes and use compare-and-set status transitions, so a callback cannot exchange one code twice. Discovered accounts preserve stable identity across consent sessions while encrypting provider hashes, UIDs, and display hints. Diagnostic and disconnect runs use recoverable 15-minute leases. Each response is committed as AES-256-GCM ciphertext before decoding; receipt finalization, source revision disposition, and encrypted continuation advancement are atomic. Ciphertext expires after 30 days. JWTs, private keys, authorization codes, and plaintext session/account/cursor data are never persisted.

Migration `0011` adds the production synchronization projection without duplicating the Stage 8.1 receipt boundary. Immutable normalized balance and transaction observations retain safe source/revision identity and receipt lineage; closed coverage intervals are recorded only after a fully drained scan. Sync state selects `longest` for a new session and `default` thereafter. Canonical import/correction lineage, pending/booked and transfer candidates, owner activation evidence, and booked-balance reconciliation are owner-scoped. Three pg-boss queues provide daily dispatch, per-generation sync, and dead-letter durability. Canonical commands remain atomic with audit, one input-version increment, recalculation record, and job; replay creates none. Provider correction is append-only compensation plus optional replacement. Account-scoped purge remains a separate, explicit Stage 8 boundary.
